...
let nameGreeting = nameCheck(`Kyson`)
...
let bob = bigOrSmall(bigOrSmallArray)
console.log(bob)
...
function sentence (sampleString) {
...
function buyChocolateFrog (gold) {
  let frog = 3
  if (gold >= 3) {
    let frogs = Math.floor(gold / frog)
    return frogs
  }
}

let totalFrogs = buyChocolateFrog(21)
...
function buyChocolateFrog2 (gold2) {
  let frog2 = 3
  if (gold2 >= 3) {
    let frogs2 = Math.floor(gold2 / frog2)
    return frogs2
  }
}

let totalFrogs2 = buyChocolateFrog2(7)
console.log(`Total frogs = ${totalFrogs2}`)
...
//CODE HERE
  ...
  let arrayIsAscending = steve(sampleArray)
